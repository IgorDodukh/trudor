...
import 'package:eshop/data/data_sources/remote/delivery_info_firebase_data_source.dart';
...
class DeliveryInfoRepositoryImpl implements DeliveryInfoRepository {
  final DeliveryInfoFirebaseDataSource firebaseDataSource;
  ...
  @override
  Future<Either<Failure, DeliveryInfo>> getRemoteDeliveryInfo() async {
    if (await networkInfo.isConnected) {
      if (await userLocalDataSource.isTokenAvailable()) {
        try {
          final String token = await userLocalDataSource.getToken();
          final result = await firebaseDataSource.getDeliveryInfo(token);
    ...
    if (await userLocalDataSource.isTokenAvailable()) {
      try {
        final DeliveryInfoModel deliveryInfo =
            await firebaseDataSource.addDeliveryInfo(params);
  ...
  Future<Either<Failure, DeliveryInfo>> editDeliveryInfo(
    ...
    if (await userLocalDataSource.isTokenAvailable()) {
      try {
        final DeliveryInfoModel deliveryInfo =
            await firebaseDataSource.editDeliveryInfo(params);
        await localDataSource.updateDeliveryInfo(deliveryInfo);
        return Right(deliveryInfo);
      } on Failure catch (failure) {
        return Left(failure);
