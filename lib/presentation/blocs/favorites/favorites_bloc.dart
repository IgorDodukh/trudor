...
class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  ...
  void _onGetFavorites(GetFavorites event, Emitter<FavoritesState> emit) async {
    ...
    try {
      ...
      syncResult.fold(
        (failure) =>
            emit(FavoritesError(favorites: state.favorites, failure: failure)),
        (favorites) => emit(FavoritesLoaded(favorites: favorites)),
      );
    } catch (e) {
      EasyLoading.showError("Failed to get Favorites: $e.\nState: $state");
      emit(FavoritesError(
          failure: ExceptionFailure(), favorites: state.favorites));
    }
  }
  ...
  void _onAddToFavorites(AddProduct event, Emitter<FavoritesState> emit) async {
    ...
    try {
      ...
      final index = favorites.indexWhere((favoritesItem) =>
          favoritesItem.product.id == event.favoritesItem.product.id);
      if (index != -1) {
        ...
        result.fold(
          (failure) => emit(
              FavoritesError(favorites: state.favorites, failure: failure)),
      ...
      emit(FavoritesError(
          favorites: state.favorites, failure: ExceptionFailure()));
    }
  }

  void _onRemoveFromFavorites(
      RemoveProduct event, Emitter<FavoritesState> emit) async {
    emit(FavoritesLoading(favorites: state.favorites));
    try {
      List<ListViewItem> favorites = [];
      favorites.addAll(state.favorites);
      final indexToRemove = favorites.indexWhere((favoritesItem) =>
          favoritesItem.product.id == event.favoritesItem.product.id);
